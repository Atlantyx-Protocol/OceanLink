...
describe("runAlgorithm", () => {
  ...
  it("single cycle below threshold — not captured", () => {
    ...
    // ratio = (5 − 3) / 5 = 0.4  ≤  x=0.5  → not processed
    const edges = buildGraph([
      [0, 1, 5],
      [1, 2, 4],
      [2, 0, 3],
    ...
    const result = runAlgorithm(3, edges, 0.5);
    assert.equal(result.length, 0, "ratio 0.4 ≤ 0.5 should not be captured");
    // edges untouched
  ...
  it("single qualifying cycle — captured once, min edge removed", () => {
    ...
    // ratio = (10 − 2) / 10 = 0.8  >  x=0.5  → process
    ...
    assertCycle(result[0]!, [[0, 1, 10], [1, 2, 2], [2, 0, 6]], "cycle 0");
  ...
  it("two qualifying cycles in sequence — both captured", () => {
    ...
    const result = runAlgorithm(4, edges, 0.3);

    assert.equal(result.length, 2, "should capture two cycles");
    assertCycle(result[0]!, [[0, 1, 10], [1, 0, 2]], "cycle 0");
    assertCycle(result[1]!, [[1, 2, 8], [2, 1, 4]], "cycle 1");
  ...
  // -------------------------------------------------------------------------
  it("self-loop — ratio is always 0, never captured", () => {
    // Self-loop 0→0(7): max=min=7, ratio=0  ≤  any positive x
    const edges = buildGraph([[0, 0, 7]]);
    const result = runAlgorithm(1, edges, 0.1);
    assert.equal(result.length, 0, "self-loop ratio=0 should never qualify");
    assert.equal(edges.length, 1, "edge untouched");
  });

  // -------------------------------------------------------------------------
  it("all edges in cycle have equal weight — ratio 0, never captured", () => {
    // 0→1(5) → 1→2(5) → 2→0(5): ratio = 0  ≤  x=0.5
    const edges = buildGraph([
      [0, 1, 5],
      [1, 2, 5],
      [2, 0, 5],
  ...
  it("disconnected graph — cycle in second component captured", () => {
    ...
    // Component B: 2→3(9), 3→2(1)  ratio=(9−1)/9 ≈ 0.89 > 0.5
    const edges = buildGraph([
      [0, 1, 4],
      [2, 3, 9],
      [3, 2, 1],
    ]);
    const result = runAlgorithm(4, edges, 0.5);
  ...
  it("x=0 — every cycle with unequal weights is captured", () => {
    ...
    // Triangle 0→1(3), 1→2(1), 2→0(2)  ratio=(3−1)/3 ≈ 0.67 > 0
    const edges = buildGraph([
      [0, 1, 3],
      [1, 2, 1],
      [2, 0, 2],
    ...
    assertCycle(result[0]!, [[0, 1, 3], [1, 2, 1], [2, 0, 2]], "cycle 0");
  });

  // -------------------------------------------------------------------------
  it("snapshot preserves original weights, not post-update weights", () => {
    // 0→1(6), 1→0(2)  ratio=(6−2)/6 ≈ 0.67 > 0.5
    ...
    const result = runAlgorithm(2, edges, 0.5);
    assert.equal(result.length, 1);
    const snap = result[0]!;
